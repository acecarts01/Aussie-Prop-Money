...
- **LIVE** at https://australianreserveprops.com — Vercel project `aussie-prop-money` in the **ACE** team (acecarts01). Apex is primary; `www` 308-redirects to apex. There is exactly one project. A stray second project named `australian-reserve-props` was briefly created in the same team (2026-09-16, when a local `.vercel` link was missing) and several deploys went there by mistake before the link was corrected — it has since been deleted. Always confirm `.vercel/project.json` says `"projectName":"aussie-prop-money"` before running `vercel --prod`.
