...
- **LIVE** at https://australianreserveprops.com — Vercel project `aussie-prop-money` in the **ACE** team (acecarts01). Apex is primary; `www` 308-redirects to apex. The old duplicate project in the Prop Money team has been deleted — there is exactly one project now. **Caution (2026-09-16):** a stray second project named `australian-reserve-props` got created in the same team when a local `.vercel` link was missing — it does NOT own the domain and several early deploys went there by mistake before the link was corrected. It's still sitting in the ACE team unused; ask the user before deleting it, but never deploy to it — always confirm `.vercel/project.json` says `"projectName":"aussie-prop-money"` before running `vercel --prod`.
...
- Email design: `src/lib/mail-templates.js` — rewritten 2026-09-16 to a single fixed dark "cinema" brand design (SVG treasury-shield logo, #0B0B0C/#141416/#35D07F palette) matching the on-site admin views; the old cinema/studio/ticket model switcher and `MAIL_TEMPLATE` selection are gone (the function still accepts a `model` param for back-compat but ignores it). Every `<table>`/`<td>` carries an explicit `bgcolor` + `background-color` (not just CSS `background`) because Outlook/Apple Mail default an unstyled nested table to opaque white, which was silently defeating the dark theme — see the comment at the top of `shell()` in that file before touching colors again. Every order sends TWO designed emails at once: the internal notification (to ORDER_TO, styled like the admin "incoming order" card, with a **View Order in Admin →** button deep-linking to `/admin/invoice/?ref=...&name=...&items=...` etc. so the owner's invoice form arrives pre-filled) and the customer confirmation (with `ARP-…` reference; it acknowledges the selected payment method but carries NO payment details — client flow 2026-09-12: the owner confirms the method first, then sends the tax invoice with details).
